/**
 * Database schema and the versioned migration list — spec §5, §11 ("versioned migration runner
 * from day one") and §16.4 ("every schema change goes through the migration runner, never a
 * hand-edit of the create statements").
 *
 * Ported unchanged from the React Native tree, because it is plain SQL with no platform in it.
 * The runner that applies these lives in `sqlite.service.ts`; keeping the statements in a
 * dependency-free module is what lets them be executed against Node's own SQLite in a test.
 *
 * The rule this file exists to enforce: `MIGRATIONS` is append-only. To change the schema you
 * add an entry; you never edit an existing one, because a released build has already run it on
 * a real device holding the owner's only copy of their invoices.
 */
 ...
 *  - Money columns are INTEGER, holding paise. There is not a REAL column anywhere in this
 *    file, and there must never be one.
 *  - `client_snapshot` / `business_snapshot` exist because §5.4 marks them required: an issued
 *    document must never change because a client was later edited.
 *  - `line_items.document_id` cascades, so deleting a document cannot leave orphan lines.
 */
 ...
 *
 * SPEC GAP, carried over from the React Native tree: §9.2 defines a `flat` tax mode as "one Tax
 * row at a single rate applied to taxBase", but §5.4 provides no column to store that rate, so
 * the mode was unimplementable as specified. Added here rather than by editing migration 1, per
 * §16.4.
 */
...
/** Tables a backup copies, ordered so foreign keys are satisfied on the way back in. */
