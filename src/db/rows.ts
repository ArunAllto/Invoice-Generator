/**
 * Row types and the mapping between SQLite's loose value types and the app's strict ones.
 *
 * SQLite has no boolean and no enum, so every 0/1 flag and every union-typed TEXT column
 * has to be narrowed on the way out. Doing that here — once, in typed mappers — is what
 * lets the rest of the app treat a `DocumentRow` as trustworthy. A row that came from a
 * restored backup written by an older build is exactly the case these mappers defend
 * against.
 */
 ...
 *
 * A malformed snapshot must not make a document unopenable — the owner would lose access
 * to a real invoice over a syntax error. Bad JSON degrades to the fallback instead.
 */
 ...
 *
 * A document saved by an earlier build will not have keys added later, and the renderer
 * must not read `undefined` as "off" — that would silently drop the client block from an
 * old invoice the first time it was re-exported.
 */
